...
export default function ( polygons, colorData, params ) {
	polygons.forEach( function ( polygon, polygonIndex ) {
		...
		'abc'.split( '' ).forEach( function ( key ) {
			let color = getColorByPos( polygon[key], colorData, params.transparent );
			...
			data[key].medianColor = getColorByPos( data[key].median, colorData, params.transparent );
		...
		polygon.gradient = {
			...
			colors: gradienStopPositions.map( function ( pos ) { return getColorByPos( pos, colorData, params.transparent); } )
		};

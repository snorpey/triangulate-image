...
function triangulateImage ( imgPath, callback ) {
	fs.readFile( __dirname + '/' + imgPath, function ( err, src ){
		...
		var blurredImageData = superfastBlur( imageData, values.blur, false );
		var greyscaleImageData = greyscale( blurredImageData );
		var edgesImageData = detectEdges( greyscaleImageData );

		var edgePoints = getEdgePoints( edgesImageData, 50, values.accuracy );
		var edgeVertices = getRandomVertices( edgePoints, values.pointRate, values.pointCount, values.accuracy, canvas.width, canvas.height );
...
triangulateImage( 'images/lincoln.jpg', function ( triangles, size ) {
